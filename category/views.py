...
def index(request, id):
    ...
    return render(request, 'views/products_display.html', {'section_title': required_category.name, 'products':products, 'categories':Category.objects.all(), 'checked':'min' if price_filter == 'price' else 'max', 'brands':Brand.objects.all(), 'selected_brands':brands_filter})